...
DEFAULT_PROMPTS = {
    PromptTemplate.BASIC_RAG: (
        ...
        "Output ONLY the exact answer — a single word, name, number, or short phrase (no more than 5 words). "
        "Do NOT add any explanation, reasoning, parenthetical notes, or extra punctuation. "
    ...
    PromptTemplate.CHAIN_OF_THOUGHT_RAG: (
        "Answer the question based on the given contexts. "
        "Think step by step, then provide ONLY the final answer as a single word, name, number, or short phrase.\n\n"
        "Contexts:\n{contexts}\n\n"
        "Question: {question}\n"
    ...
    PromptTemplate.ZERO_SHOT: (
        "Answer the following question with ONLY the answer itself — a single word, name, number, or short phrase. "
        "Do NOT explain.\n\n"
        "Question: {question}\n"
        "Answer:"
    ),
    PromptTemplate.REACT_RAG: (
        ...
        "- The final Answer should be as precise and concise as possible — a single word, name, number, or short phrase.\n"
        "Question: {question}\n"
        "Contexts:\n{contexts}\n"
        "Thought:"
    ),
