...
class AutoReasonSearchWorkflow(BaseWorkflow):
    ...
    class Configuration(BaseWorkflowConfiguration):
        ...
        # Rankify reranker configuration
        ranker_preset: Optional[str] = None  # e.g., "rankr1-7b", "bge-reranker-large", "setr", "rank4gen"
        ranker_top_n: int = -1  # Top N docs to keep after reranking (-1 = all, ignored for setr/rank4gen)
        ranker_timeout: int = 300  # Timeout for ranker MCP call (seconds)
    ...
    def setup_components(
        ...
        # If a ranker preset is specified, wrap search tools with ChainedTool(search + rerank)
        if cfg.ranker_preset:
            ranker_tool = RankifyRerankerTool(
                preset=cfg.ranker_preset,
                tool_parser=cfg.tool_parser,
                top_n=cfg.ranker_top_n,
                timeout=cfg.ranker_timeout,
                transport_type=mcp_transport_type,
                mcp_executable=mcp_executable,
                mcp_port=mcp_port,
            )
            self.search_tool = ChainedTool(
                [self.search_tool, ranker_tool],
                name="snippet_search",
                tool_parser=cfg.tool_parser,
                output_formatting="last",
            )
            self.search_tool2 = ChainedTool(
                [self.search_tool2, RankifyRerankerTool(
                    preset=cfg.ranker_preset,
                    tool_parser=cfg.tool_parser,
                    top_n=cfg.ranker_top_n,
                    timeout=cfg.ranker_timeout,
            ...
            print(f"Using Rankify reranker: preset={cfg.ranker_preset}, top_n={cfg.ranker_top_n}")
