...
def _serper_via_internal_proxy(payload: dict, timeout: int = None) -> dict:
    """Call Serper search via internal proxy."""
    url = f"{INTERNAL_PROXY_BASE_URL}/search"
    _timeout = timeout or INTERNAL_PROXY_TIMEOUT
    auth = f"Bearer {INTERNAL_PROXY_APP_ID}:{INTERNAL_PROXY_APP_KEY}?provider=serper&timeout={_timeout}"
    headers = {
        "Authorization": auth,
        "Content-Type": "application/json",
    }
    response = requests.post(url, headers=headers, json=payload, timeout=_timeout + 10)
    if response.status_code != 200:
        raise Exception(
            f"Internal proxy Serper request failed with status {response.status_code}: {response.text}"
        )
...
def search_serper(
    ...
    if USE_INTERNAL_PROXY and INTERNAL_PROXY_APP_ID and INTERNAL_PROXY_APP_KEY:
        try:
            return _serper_via_internal_proxy(payload)
        except requests.exceptions.RequestException as e:
            raise Exception(f"Error performing Serper search via internal proxy: {str(e)}")
...
def search_serper_scholar(
    ...
    if USE_INTERNAL_PROXY and INTERNAL_PROXY_APP_ID and INTERNAL_PROXY_APP_KEY:
        try:
            ...
            raise Exception(f"Error performing Serper scholar search via internal proxy: {str(e)}")
...
def fetch_webpage_content(
    ...
    """
    # Note: Serper scrape endpoint doesn't have a known internal proxy equivalent.
    # If you need webpage scraping via internal proxy, use Jina browse tool instead.
    if not api_key:
        api_key = os.getenv("SERPER_API_KEY")
        if not api_key:
            raise ValueError(
