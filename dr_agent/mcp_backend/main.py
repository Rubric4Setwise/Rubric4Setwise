...
from .apis.rankify_reranker_apis import RankifyRerankerResult
...
from .cache import set_cache_enabled, init_cache_for_port
...
@mcp.tool(tags={"rerank"})
def rankify_reranker(
    query: Annotated[str, "Search query string to rank documents against"],
    documents: Annotated[List[str], "List of document texts to rerank"],
    preset: Annotated[
        str,
        "Ranker preset name. Available: bge-reranker-large, rankllama, monot5, "
        "rankvicuna, rankzephyr, rankgpt, rankt5, rankr1-7b, rank1-7b, "
        "rearank-7b, reasonrank-7b, setwise-sft-7b, setr, rank4gen, "
        "rubric4setwise, rubric4setwise-llama8b",
    ],
    top_n: Annotated[
        int, "Number of top documents to return. -1 returns all. Ignored for set selection methods (setr, rank4gen, rubric4setwise*)."
    ] = -1,
) -> RankifyRerankerResult:
    """
    Rerank documents using Rankify ranker models.

    Supports multiple ranker types:
    - Cross-Encoder: bge-reranker-large (550M)
    - Pointwise: rankllama (7B), monot5 (3B), rankt5 (3B), rank1-7b (7B)
    - Listwise: rankvicuna (7B), rankzephyr (7B), rankgpt (8B), reasonrank-7b (7B), rearank-7b (7B)
    - Setwise HeapSort: rankr1-7b (7B), setwise-sft-7b (7B)
    - Set Selection: setr (8B), rank4gen (8B)

    For set selection methods (setr, rank4gen), the model selects a relevant subset
    rather than producing a full ranking. The top_n parameter is ignored.

    Returns:
        RankifyRerankerResult with method, model_name, preset, result_type, and ranked results.
    """
    from dr_agent.mcp_backend.apis.rankify_reranker_apis import rankify_rerank

    result = rankify_rerank(
        query=query,
        documents=documents,
        preset=preset,
        top_n=top_n,
...
if __name__ == "__main__":
    ...
    if args.no_cache:
        ...
        # Use port-specific cache directory to avoid SQLite lock conflicts
        # when running multiple MCP servers on different ports
        init_cache_for_port(args.port)
