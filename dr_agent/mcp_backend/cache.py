...
class ApiCache:
    ...
    def __init__(
        ...
        # timeout=60: wait up to 60 seconds for SQLite lock to avoid "locking protocol" errors
        # Auto-recover from corrupted cache by deleting and retrying
        cache_path = str(self.cache_dir / "diskcache")
        for attempt in range(2):
            try:
                self.cache = Cache(cache_path, timeout=60)
                return
            except sqlite3.OperationalError as e:
                logger.warning(
                    f"Failed to open cache at {cache_path} (attempt {attempt + 1}/2): {e}"
                )
                if attempt == 0:
                    logger.warning("Deleting corrupted cache and retrying...")
    ...
    def get(self, cache_key: str, cache_ttl: int = None) -> Optional[Dict[str, Any]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Cache get attempt {attempt + 1}/{max_retries} failed for key {cache_key}: {e}"
                )
                if attempt < max_retries - 1:
                    time.sleep(0.5 * (attempt + 1))
                else:
                    logger.error(f"Cache get failed after {max_retries} retries: {e}")
                    return None
    ...
    def set(self, cache_key: str, data: Dict[str, Any]) -> None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Cache set attempt {attempt + 1}/{max_retries} failed for key {cache_key}: {e}"
                )
                if attempt < max_retries - 1:
                    time.sleep(0.5 * (attempt + 1))
        logger.error(f"Failed to set cache for key {cache_key} after {max_retries} retries")
        raise RuntimeError(f"Failed to set cache for key {cache_key} after {max_retries} retries")
...
_DEFAULT_CACHE_BASE_DIR = os.getenv(
    "MCP_CACHE_DIR", str(Path(__file__).resolve().parents[2] / ".cache")
)

# Create a global cache instance
DEFAULT_CACHE = ApiCache(
    cache_dir=_DEFAULT_CACHE_BASE_DIR,
    cache_ttl=86400 * 10,  # Default: 10 days cache
)


def init_cache_for_port(port: int) -> None:
    """
    Reinitialize DEFAULT_CACHE with a port-specific cache directory.
    This allows multiple MCP servers on different ports to have independent
    SQLite cache files, avoiding "locking protocol" errors from diskcache.
    """
    global DEFAULT_CACHE
    cache_dir = str(Path(_DEFAULT_CACHE_BASE_DIR) / f"port_{port}")
    logger.info(f"Initializing cache for port {port} at: {cache_dir}")
    DEFAULT_CACHE = ApiCache(cache_dir=cache_dir, cache_ttl=86400 * 10)
    logger.info(f"DEFAULT_CACHE reinitialized for port {port}")
