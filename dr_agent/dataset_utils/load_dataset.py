...
def load_dataset(config: DatasetConfig) -> List[Dict]:
    ...
    elif config["name"] == "custom_json":
        return load_custom_json_data(local_path, num_examples, shuffle)
    else:
        # Try to load as a local JSON/JSONL file path
        if config["name"].endswith((".json", ".jsonl")):
            return load_custom_json_data(config["name"], num_examples, shuffle)
        raise ValueError(
            f"Unsupported dataset: {config['name']}. Supported datasets: {list(SUPPORTED_TASKS.keys())}, browsecomp, simpleqa, healthbench, researchqa, deep_research_bench, custom_json"
        )
...
def load_custom_json_data(
    file_path: str,
    num_examples: Optional[int] = None,
    shuffle: bool = False,
) -> List[Dict]:
    """
    Load a custom local JSON/JSONL file with ResearchQA-compatible format.
    
    Expected format (JSON array or JSONL):
    Each item should have at minimum: {"id": "...", "query": "..."}
    Optionally: {"rubric": [...], "general_domain": "...", ...}
    
    The function converts each item to the workflow-expected format:
    {"id": "...", "problem": "query text", ...}
    """
    ...
    examples = []
    for sample in data:
        # Map "query"/"question" to "problem" (workflow expected field)
        query = sample.get("query", sample.get("question", ""))
        item_id = sample.get("id", sample.get("_id", hashlib.md5(query.encode()).hexdigest()))
        
        examples.append({
            "id": str(item_id),
            "problem": query,
            "additional_instructions": sample.get(
                "additional_instructions",
                "Answer the question completely and precisely."
            ),
            # Preserve original fields for evaluation
            **{k: v for k, v in sample.items() if k not in ("query", "question", "id", "_id")}
        })
    ...
    print(f"[Custom JSON] Loaded {len(examples)} examples from {file_path}")
    return examples
