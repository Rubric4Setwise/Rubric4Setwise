...
class MCPMixin:
    ...
    def init_mcp_client(self):
        ...
        # Clear proxy environment variables to prevent httpx (used internally by
        # StreamableHttpTransport) from routing local MCP connections through
        # external HTTP proxy. httpx does not reliably honor NO_PROXY/no_proxy.
        for proxy_var in (
...
class RankifyRerankerTool(MCPRerankerTool):
    """
    Tool for reranking documents using Rankify ranker models via MCP.

    Supports all Rankify presets:
    - Cross-Encoder: bge-reranker-large
    - Pointwise: rankllama, monot5, rankt5, rank1-7b
    - Listwise: rankvicuna, rankzephyr, rankgpt, reasonrank-7b, rearank-7b
    - Setwise HeapSort: rankr1-7b, setwise-sft-7b
    - Set Selection: setr, rank4gen (these ignore top_n and select a relevant subset)
    """

    # Presets that do set selection instead of full ranking
    SET_SELECTION_PRESETS = {"setr", "rank4gen",
                             "rubric-maxcov-v5-qwen8b", "rubric-maxcov-v5-llama8b"}
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Args:
            preset: Rankify ranker preset name (e.g., "rankr1-7b", "bge-reranker-large").
            tool_parser: Tool call parser instance or name.
            top_n: Number of top documents to return (-1 for all).
                   Ignored for set selection presets (setr, rank4gen).
            timeout: MCP call timeout in seconds (default 300 for slow rankers).
        ...
        super().__init__(
            ...
            timeout=timeout,
            top_n=top_n,
            name=name or "rankify_reranker",
            score_threshold=score_threshold,
            **kwargs,
        )
        self.preset = preset
        self._is_set_selection = preset in self.SET_SELECTION_PRESETS

    def get_mcp_tool_name(self) -> str:
        return "rankify_reranker"

    def get_mcp_params(
        self, query: str, documents: List[str], top_n: int
    ) -> Dict[str, Any]:
        """Build parameters for Rankify reranker MCP call."""
        params = {
            "query": query,
            "documents": documents,
            "preset": self.preset,
            "top_n": -1 if self._is_set_selection else top_n,
        }
        return params
    ...
    def _process_reranker_results(
        ...
        """
        Process Rankify reranker results and create updated Document list.
        Handles both ranking results and set selection results.
        """
        reranked_results = raw_output.get("results", [])
        if not reranked_results:
            return []
        ...
        for result in reranked_results:
            if isinstance(result, dict):
                doc_index = int(result.get("index", -1))
                rerank_score = result.get("relevance_score", 0.0)

                if 0 <= doc_index < len(original_documents):
                    original_doc = original_documents[doc_index]
